...
some initial setup, [[https://en.wikipedia.org/wiki/Literate_programming][tangle]] the source blocks in this file and execute
them. The output will be =config.el= which I don't track in git.
...
* notes
** org mode
+ [[https://orgmode.org/worg/org-contrib/][org-contrib]] lists some  more org mode funtionality that might be
  worth checking out.
...
This needs to come first to bootstrap [[https://github.com/jwiegley/use-package#key-binding][use-package]].

Install use-package and set =ensure= true to make all uses of
** intial
