...
* purescript
  ...
  #+begin_src emacs-lisp
    ...
    (defun forall-replace ()
      "Replace all occurances of 'forall' with '∀'."
	(save-excursion
	  (goto-char (point-min))
	  (while (re-search-forward "forall" nil t)
	    (replace-match "∀"))))
  #+end_src
...
ghcide setup, from https://github.com/digital-asset/ghcide/blob/39605333c34039241768a1809024c739df3fb2bd/README.md
...
use `C-c C-c` to apply LSP suggestions in lsp-ui-mode
#+begin_src emacs-lisp
  (add-hook 'lsp-ui-mode-hook (lambda () (local-set-key (kbd "C-c C-c") 'lsp-ui-sideline-apply-code-actions)))
#+end_src

** defunct
*** haskell lsp-mode

I first tried replacing intero-mode with haskell lsp-mode, but the
interface was clunky and most of the advertised features didn't seem
to work
...
*** intero-mode
- unfortunately intero has been end-of-lifed, so everything in this
section is useless. I still prefer it to the haskell lsp mode, so it's
a real bummer...
...
couldn't get dante mode working
